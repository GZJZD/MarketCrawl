...
class CompanyNewSpider(Spider):
    ...
    def spider_opened(self, spider):
        ...
        if self.db_connect is not None:
            ...
            sql = '''SELECT shares_code, MAX(news_id) FROM crawler_company_news 
            ...
            for feild in result:
                assert isinstance(feild, tuple)
                self.last_news_id[feild[0]] = feild[1]
    ...
    def parse_page_data(self, response):
        ...
        if 'list' in json_obj and len(json_obj['list']) > 0:
            ...
            for unit in page_data:
                ...
                # 转换为北京时间
                item['date'] = self.transfrom_beijing_time(string.atoi(str(unit['created_at'])))

                # 获取公告的ID
                news_id = string.atoi(str(unit['id']))
                item['news_id'] = news_id

                if min_news_id > news_id:
                    min_news_id = news_id
        ...
        if cur_code in self.last_news_id:
            cur_last_news_id = self.last_news_id[cur_code]
        else:
            cur_last_news_id = 0
        ...
        elif self.partial_update and min_news_id < cur_last_news_id:
